...
class NhanSuService {
  ...
  Stream<NhanVien?> getNhanVienByAuthUid(String authUid) {
    return _ns.where('authUid', isEqualTo: authUid)
        .limit(1)
        .snapshots()
        .map((snapshot) {
      if (snapshot.docs.isNotEmpty) {
        return NhanVien.fromMap(
  ...
  Stream<List<NhanVien>> streamNhanSu() {
    return _ns.snapshots().map((snapshot) {
